...
/* 1.Analiza el plan de ejecucion de la siguiente consulta y optimizala para que use indices y particiones.

SELECT c.Nombre, COUNT(p.PedidoID) AS TotalPedidos FROM Clientes c, Pedidos p
...
EXPLAIN PLAN FOR
SELECT c.Nombre, COUNT(p.PedidoID) AS TotalPedidos FROM Clientes c, Pedidos p
WHERE c.ClienteID = p.ClienteID AND c.Ciudad = 'Santiago' AND p.FechaPedido >= TO_DATE('2025-03-01', 'YYYY-MM-DD')
GROUP BY c.Nombre;
...
WHERE c.Ciudad = 'Santiago' AND p.FechaPedido >= TO_DATE('2025-03-01', 'YYYY-MM-DD')
GROUP BY c.Nombre;
SELECT * FROM TABLE(DBMS_XPLAN.DISPLAY);

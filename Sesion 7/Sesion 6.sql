...
--- Practica Sesion 6
CREATE OR REPLACE TYPE obj_producto_info AS OBJECT (
    ---Crear tabla de objetos
    nombre_producto_obj VARCHAR2(50),
    precio_producto_obj NUMBER
);
/
DECLARE
    ---Creacion del cursor
    CURSOR c_producto_obj IS
        SELECT obj_producto_info(Nombre, Precio) FROM Productos
        ORDER BY Precio ASC;
    v_producto_obj obj_producto_info;
BEGIN
    OPEN c_producto_obj ---Abrimos el cursor

    LOOP --- Loop para recorrer la tabla
    FETCH c_producto_obj into v_producto_obj

    EXIT WHEN c_producto_obj $ notfound ---Condicion de salida sugerida

    --- Se cierra el cursor
    CLOSE c_producto_obj;
END;
/
--- Sesion 6, requisito 2
--- Creacion del objeto
CREATE OR REPLACE TYPE obj_producto_calc AS OBJECT (
    id_producto NUMBER,
    nombre VARCHAR2(50),
    precio NUMBER,
    --- la función que hará el cálculo
    MEMBER FUNCTION obtener_precio_aumentado RETURN NUMBER
);
/

--- Declaración del cuerpo del objeto 
CREATE OR REPLACE TYPE BODY obj_producto_calc AS
    MEMBER FUNCTION obtener_precio_aumentado RETURN NUMBER IS
    BEGIN
        -- Multiplicar por 1.10 equivale a sumar un 10%
...
DECLARE
    --- Cursor explícito basado en un objeto
    CURSOR c_productos (p_precio_minimo NUMBER) IS
        SELECT obj_producto_calc(ProductoID, Nombre, Precio)
        FROM Productos
        WHERE Precio >= p_precio_minimo
        FOR UPDATE; 

    --- Variables
    v_prod             obj_producto_calc;
    v_precio_nuevo     NUMBER;
    
    -- Variables para llevar "el total de la suma" que pide el requisito
    v_suma_total_orig  NUMBER := 0;
    v_suma_total_nueva NUMBER := 0;
BEGIN
    --- Abrimos el cursor 
    OPEN c_productos(0);
    
    LOOP
        --- Extraemos la fila directamente como un objeto
        FETCH c_productos INTO v_prod;
        EXIT WHEN c_productos%NOTFOUND;
        
        --- Usamos la función interna del objeto para obtener el nuevo valor
        v_precio_nuevo := v_prod.obtener_precio_aumentado();
        
        --- Acumulamos los valores para calcular la suma total de este atributo numérico
        v_suma_total_orig  := v_suma_total_orig + v_prod.precio;
        v_suma_total_nueva := v_suma_total_nueva + v_precio_nuevo;
        
        --- Actualizamos la tabla física de forma segura
        --- WHERE CURRENT OF asegura que solo se actualice la fila que el cursor acaba de leer
        UPDATE Productos
        SET Precio = v_precio_nuevo
